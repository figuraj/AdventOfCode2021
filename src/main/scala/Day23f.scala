...
object Day23f extends App {
  ...
  case class A(atype: Char, position: (Int,Int), id: Int){
  ...
  def moveSingleAmphipod(amphipod: A, others: List[A], init_cost: Int): List[(Game, Int)] = {
    filterOutMoves(amphipod, others)
      ...
      .map(x => Game((amphipod.copy(position = x._1) :: others).toSet) -> (init_cost + x._2*amphipod.cost))
  }
  ...
  def solve(game: Game, end: Set[((Int,Int),Char)]): Int = {
    @tailrec
    def iter(prio_queue: Set[Game], expanded: Map[Game, Int]): Int = {
      //println(prio_queue)
      if (expanded.exists(x => x._1.state.map(x => (x.position, x.atype)) == end)) {
        expanded.filter(x => x._1.state.map(x => (x.position, x.atype)) == end).minBy(_._2)._2
      } else {
  ...
  val end = Set(((2,1),'A'),((2,2),'A'),((4,1),'B'),((4,2),'B'),((6,1),'C'),((6,2),'C'),((8,1),'D'),((8,2),'D'))
  ...
  val start = Game(Set(A('A',(2,1),1),A('A',(4,2),2),A('B',(4,1),3),A('B',(2,2),4),A('C',(6,1),5),A('C',(6,2),6),A('D',(5,0),7),A('D',(8,2),8)))
